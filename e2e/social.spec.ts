...
test.describe.serial("follow, like, comment, feed, notifications", () => {
  ...
  test("Alice is notified of the follow, like, and comment", async ({ browser }) => {
    const page = await browser.newPage();
    await signIn(page, USERS.alice);
    ...
    await expect(page.getByText("commented on your review")).toBeVisible();
    await page.close();
  });
  ...
  test("Alice edits her profile display name", async ({ browser }) => {
    const page = await browser.newPage();
    await signIn(page, USERS.alice);
    await page.goto("/me"); // resolves to /u/[handle]
    await page.getByRole("button", { name: "Edit profile" }).click();
    const nameInput = page.getByLabel("Display name");
    await nameInput.fill("Alice in Cinemas");
    await page.getByRole("button", { name: "Save" }).click();
