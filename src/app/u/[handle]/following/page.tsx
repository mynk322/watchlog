...
export default async function FollowingPage({ params }: { params: Promise<{ handle: string }> }) {
  ...
  const following = await getFollowing(profile.userId, userId);
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-16 pt-24 sm:px-8">
      ...
      <h1 className="mt-3 text-2xl font-bold text-foreground">Following</h1>

      <div className="mt-6 flex flex-col gap-3">
        {following.length === 0 ? (
          <p className="text-sm text-muted">Not following anyone yet.</p>
        ) : (
          following.map((user) => <UserListItem key={user.userId} user={user} />)
        )}
      </div>
    </div>
  );
